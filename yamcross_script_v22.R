...
#setwd("C:/Users/Margaret/Dropbox/YAM/Yam-Cross-master/yamcross1/data")
...
yam = koboloadeR::kobo_data_downloader("formid", "accountname:password", api="ona")  %>%
...
bagcode = dplyr::select(yam, ends_with("GetSite"), ends_with("PlantCode"),ends_with("GetAccessionName"), ends_with("GetPlantingDate"),
                        ends_with("BaggingDate"), ends_with('BagsDay'), ends_with("/bag_number")) %>%
  ...
  gather("id","Day", ends_with("BagsDay"), na.rm = F)  %>% 
  gather("id","BagCode", ends_with("/bag_number"), na.rm=F) %>% 
  dplyr::select(-starts_with("id"))  %>%
  .[!duplicated(.[,2:7]),] %>%
  dplyr::filter(!is.na(BagCode)) 

assignedId = dplyr::select(yam,ends_with("GetSite"), ends_with("PlantCode"),ends_with("GetAccessionName"), ends_with("GetPlantingDate"),
                           ends_with("BaggingDate"), ends_with('BagsDay'), ends_with('/assignBagId')) %>%
  gather("id","Site", ends_with("GetSite"), na.rm = F) %>% 
  gather("id","PlantName", ends_with("PlantCode"), na.rm = T) %>% 
  gather("id","Genotype", ends_with("GetAccessionName"), na.rm = T) %>% 
  gather("id","Planting_Date", ends_with("GetPlantingDate")) %>% 
  gather("id","Bagging_Date", ends_with("BaggingDate"), na.rm = T)  %>% 
  gather("id","Day", ends_with("BagsDay"), na.rm = F)  %>% 
  gather("id","BagCode", ends_with("/assignBagId"), na.rm=F) %>% 
  dplyr::select(-starts_with("id"))  %>%
  .[!duplicated(.[,2:6]),]

bagging = plyr::rbind.fill(bagcode, assignedId)
bagging = bagging %>%
  dplyr::group_by(PlantName) %>%
  dplyr::mutate(BagN=row_number())  %>%
  arrange(desc(BagN))

bagging$BagNo = sub(".*Bag","", bagging$BagCode)
bagging$BagNo = ifelse(!bagging$BagNo==bagging$BagN, bagging$BagNo, bagging$BagN)
bagging = bagging %>%
  arrange(PlantName, desc(BagNo)) %>%
  dplyr::filter(!is.na(BagCode))
bagging <- bagging[order(as.integer(bagging$BagNo),decreasing = T), ]
bagging$BagN=NULL

# spikes = dplyr::select(yam, ends_with("/Number_of_Spikes")) %>%
#   tibble::rowid_to_column() %>%
#   gather("id","Number_of_Spikes", ends_with("/Number_of_Spikes"), na.rm=F) %>% 
#   dplyr::select(-starts_with("id"))  %>%
#   arrange(rowid) %>%
#   .[complete.cases(.),]
#   
# bagging = dplyr::left_join(bagging, spikes, by='rowid') %>%
#   .[!duplicated(.[,2:9]),]  %>%
#   arrange(desc(BagNo))
# 
# rowids = data.table(table(bagging$rowid))
# rowid=data.frame(rowids[rep(seq_len(dim(rowids)[1]), rowids$N), 1, drop = FALSE], row.names=NULL)
# rowid %<>% mutate_all(as.integer)
# colnames(rowid) = 'rowid'
# 
# s = plyr::rbind.fill(spikes, rowid)
# 3
...
pollination = dplyr::select(yam, ends_with("CrossNumber"), ends_with("FemaleCode"), ends_with("FemaleAccessionName"),ends_with("FirstPollination/MaleCode"),
                            ...
                            ends_with('assignCrossId'), ends_with("PollinatorName")) %>%
  gather("id","CrossNumber", ends_with("CrossNumber"), na.rm=F) %>%
  gather("id","AssignedID", ends_with("assignCrossId"), na.rm=F) %>%
...
pollination$CrossNumber = ifelse(is.na(pollination$CrossNumber) & !is.na(pollination$AssignedID), pollination$AssignedID, pollination$CrossNumber)
pollination$AssignedID = NULL
...
while(new_family == ''){
  ...
  post.family <- postForm("https://api.ona.io/api/v1/metadata.json",
                          data_value='familydata.csv',data_type='media',xform=294772,
